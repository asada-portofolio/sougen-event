    ...
    <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
      ...
      <Dialog.Trigger asChild>
        <button
          ...
          className={cn(
            "inline-flex items-center gap-1 sm:gap-1.5 px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-md text-[9px] sm:text-[10px] font-inter font-bold uppercase tracking-wider transition-all duration-200 shadow-sm cursor-pointer border select-none max-w-full",
            isComplete 
        ...
        >
          {isComplete ? (
            <>
              ...
              <span className="truncate">Lengkap</span>
            </>
          ) : (
            <>
              <AlertTriangle className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-600 shrink-0" />
              <span className="truncate">
                <span className="hidden sm:inline">Data </span>Belum Lengkap ({missingRequired.length})
              </span>
            </>
          )}
        </button>

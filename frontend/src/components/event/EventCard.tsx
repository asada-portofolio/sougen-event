...
export function EventCard({ event }: EventCardProps) {
  ...
  return (
    <Link 
      to={`/event/${event.slug}`} 
      className="group flex flex-col bg-white border-2 border-rpo-red rounded-xl overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(254,0,0,0.12)]"
    >
      <div className="w-full aspect-[3/4] relative bg-[#f0f0f0] overflow-hidden">
        ...
        {/* Status Badge */}
        {event.isActive ? (
          <div className="absolute top-3 right-3 bg-rpo-red text-white px-3 py-1.5 text-[10px] font-black uppercase tracking-wider rounded-sm z-10 shadow-sm">
            Active
          </div>
        ) : (
          <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm text-rpo-black border border-black/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-sm z-10 shadow-sm">
            Selesai
          </div>
        )}
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        <h3 className="font-poppins text-xl font-bold text-rpo-black mb-2 line-clamp-2 group-hover:text-rpo-red transition-colors">
          {event.name}
        </h3>
        ...
        <div className={cn("space-y-2 mt-auto pt-4 border-t border-black/5", !event.theme && "mt-4")}>
          <div className="flex items-start gap-2 text-rpo-black/50 font-inter text-xs font-medium">
            <Calendar className="w-3.5 h-3.5 mt-0.5 shrink-0 text-rpo-red" />
            <span>{formatDate(event.startDate, event.endDate)}</span>
          </div>
          <div className="flex items-start gap-2 text-rpo-black/50 font-inter text-xs font-medium">
            <MapPin className="w-3.5 h-3.5 mt-0.5 shrink-0 text-rpo-red" />

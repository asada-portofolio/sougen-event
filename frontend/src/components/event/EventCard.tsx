    ...
    >
      ...
      <div className="p-3 sm:p-3.5 flex flex-col gap-2">
        <div>
          {/* Theme Tag / Hashtag */}
          {event.theme && (
            <span className="text-[10px] sm:text-[11px] font-mono font-bold text-sougen-blue uppercase tracking-wider line-clamp-1 mb-0.5 block">
              #{event.theme}
            </span>
          )}
        ...
        <div className="pt-2 border-t border-black/5 space-y-1">
          <div className="flex items-center gap-1.5 text-rpo-black/60 font-inter text-[10.5px] sm:text-[11.5px]">
            <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-sougen-blue shrink-0" />
            <span className="truncate">{formatDate(event.startDate, event.endDate)}</span>
          </div>
          <div className="flex items-center gap-1.5 text-rpo-black/60 font-inter text-[10.5px] sm:text-[11.5px]">
            <MapPin className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-sougen-blue shrink-0" />

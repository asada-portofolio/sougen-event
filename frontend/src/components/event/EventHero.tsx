                ...
                {days.length > 0 ? (
                  days.map(day => (
                    <button 
                      ...
                      type="button"
                      onClick={() => handleDayClick(day.id)}
                      className="flex-1 min-w-0 inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-xl bg-sougen-blue/10 hover:bg-sougen-blue hover:text-white border border-sougen-blue/20 font-inter font-bold text-sougen-blue text-[0.68rem] tracking-tight uppercase transition-all duration-200 active:scale-95 group cursor-pointer shadow-sm"
                      title={`Lihat jadwal rundown Day ${day.dayNumber}`}
                    >
                      <CalendarDays className="w-3 h-3 text-sougen-blue group-hover:text-white shrink-0" />
                      <span className="truncate">Day {day.dayNumber}: {formatNumericDate(day.date)}</span>
                    </button>
                  ))
                ) : (
                  <span className="inline-flex items-center gap-1 font-inter font-semibold text-gray-500 text-[0.65rem] tracking-[0.15em] uppercase">

...
export function EventRundownSection({ days, activeDayId: controlledActiveDayId, onDayChange }: EventRundownSectionProps) {
  const [internalActiveDayId, setInternalActiveDayId] = useState<number>(days?.[0]?.id || 0);

  const activeDayId = controlledActiveDayId !== undefined ? controlledActiveDayId : internalActiveDayId;
  const setActiveDayId = (id: number) => {
    setInternalActiveDayId(id);
  ...
  return (
    <section id="rundown" className="w-full bg-[#FAFAFA] py-7 sm:py-8 md:py-10 lg:py-12 px-4 lg:px-8 scroll-mt-16 md:scroll-mt-20">
      <div className="max-w-7xl mx-auto">
        ...
        {days.length > 1 && (
          <div className="mt-5 sm:mt-6 flex lg:hidden bg-white/50 rounded-xl p-1.5 border border-rpo-black/5 mx-auto max-w-[90%] shadow-sm overflow-hidden">
            {days.map((day) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "relative flex-1 py-2 text-center transition-all duration-300 font-inter font-bold uppercase text-xs tracking-wider rounded-lg",
                    isActive 
        ...
        <div className="mt-4 sm:mt-5 lg:hidden max-w-[92%] sm:max-w-[90%] mx-auto">
          <DayTimeline day={activeDay} formatDate={formatDate} getDayTimeRange={getDayTimeRange} />
        </div>
        ...
        <div className="hidden lg:block mx-auto max-w-[75%]">
          ...
          <div 
            className="grid mt-8 lg:mt-10 gap-6 lg:gap-8 items-start"
            style={{ gridTemplateColumns: `repeat(${days.length}, minmax(0, 1fr))` }}
          >
            {days.map((day) => (
              <DayTimeline key={day.id} day={day} formatDate={formatDate} getDayTimeRange={getDayTimeRange} />
            ))}
          </div>

          {/* Desktop Section Divider */}
          <div className="mt-10 md:mt-8 lg:mt-10 w-full h-1.5 md:h-2 bg-sougen-blue rounded-full" />
        </div>

        {/* Mobile Divider (outside desktop wrapper) */}
        <div className="lg:hidden mt-6 sm:mt-7 w-full h-1.5 bg-sougen-blue rounded-full mx-auto max-w-[85%]" />

# Production Readiness — Checklist Kualitas & Kesiapan Rilis Website RPO
...
## FASE PR-1 — SEO & Metadata

### PR-1.1 Tag `<title>` Unik Per Halaman
- [x] Ganti `<title>frontend</title>` di `index.html` menjadi judul default yang representatif (contoh: `Sougen Creative Management — Event Organizer Komunitas Kreatif`)
...
- [x] Verifikasi bahwa setiap pemanggilan komponen `SEO.tsx` menyertakan prop `description` yang relevan dan spesifik per halaman — bukan teks generik yang sama
- [x] Periksa panjang deskripsi: idealnya 120–160 karakter per halaman
...
- [x] Siapkan gambar OG default berukuran 1200×630px di `public/` sebagai fallback global (contoh: `og-default.jpg`)
- [x] Tes preview link dengan membagikan URL halaman ke WhatsApp / Twitter / Facebook — pastikan gambar dan teks muncul rapi
...
## FASE PR-2 — Aksesibilitas & Tampilan Dasar
...
- [x] Khusus gambar dekoratif yang murni estetis, boleh menggunakan `alt=""` — tetapi gambar dengan informasi (logo, poster, foto profil) wajib memiliki deskripsi yang relevan
- [x] Periksa komponen `ImageWithSkeleton.tsx` — pastikan fallback `alt` bukan string kosong jika tidak ada prop `alt` yang dikirim
...
## FASE PR-3 — Kualitas Kode & Performa
...
- [ ] Buka semua halaman publik satu per satu di browser → Inspect → Console
...
- [ ] Implementasi `React.lazy()` + `Suspense` untuk semua halaman di `App.tsx` — saat ini semua halaman di-import secara eager (langsung)
- [ ] Prioritaskan lazy loading untuk halaman Admin (jarang diakses pengunjung publik)
- [ ] Verifikasi bahwa bundle JS terpecah menjadi beberapa chunk setelah build (`npm run build` → periksa output `dist/assets/`)
...
- [ ] Evaluasi apakah perlu migrasi ke Next.js untuk SEO yang lebih optimal — dokumentasikan keputusan dan alasannya
...
## FASE PR-4 — Konfigurasi Bot & AI

### PR-4.1 File `robots.txt`
- [ ] Buat file `robots.txt` di `frontend/public/` dengan konfigurasi yang tepat
- [ ] Izinkan crawler mesin pencari mengakses seluruh halaman publik (`Allow: /`)
- [ ] Blokir akses ke rute admin (`Disallow: /admin`)
- [ ] Tambahkan referensi ke sitemap: `Sitemap: https://domain-anda.com/sitemap.xml`
- [ ] Tentukan kebijakan untuk bot AI (ChatGPT, Claude, dll.) — izinkan atau blokir sesuai kebutuhan
...
- [ ] Evaluasi apakah website RPO membutuhkan `llms.txt` — jika kontennya bersifat informatif publik, bisa bermanfaat
...
- [ ] Jalankan `npm run build` di frontend — pastikan build berhasil tanpa error
- [ ] Periksa ukuran output bundle — catat total ukuran `dist/assets/` sebagai baseline
- [ ] Tes semua halaman publik di 3 breakpoint: Mobile (375px), Tablet (768px), Desktop (1280px)
- [ ] Lakukan audit Lighthouse (Chrome DevTools → Lighthouse) untuk halaman Home — targetkan skor hijau (≥90) pada semua kategori: Performance, Accessibility, Best Practices, SEO
